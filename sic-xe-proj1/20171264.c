...
struct cmd *history_head=NULL;
struct cmd *history_tail=NULL;
int his_cnt=1;
int running=1;
...
//opcode function
int opcode_f(char *mnem){
	int ret=0, hash=(mnem[0]-'A')%20;
...
int command_input(){
	...
	if(strncmp(command, "help", 5)==0 || strncmp(command, "h", 2)==0){
		...
		int opcode_num=opcode_f(mnemonic_param);
		if(opcode_num==-1)	return 0;
		history_update(command, command_index);
